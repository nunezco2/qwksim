...
//! parameterised over the FLAG-C utility formula
//! `α·(deadline_slack/deadline) + β·realised_fidelity
//! + γ·(1 − rivalry) − δ·degradation_penalty` (per-category α/β/γ/δ;
//! category 5 has β = δ = 0), and the disagreement-point evaluator
//! returning each agent's payoff under the FCFS-on-stale-advertised-
//! summaries baseline (FLAG-B closed).
//!
